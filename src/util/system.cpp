...
const char * const BITCOIN_CONF_FILENAME = "fuertecoinxtreme.conf";
...
#else
    const char* pszModule = "fuertecoinxtreme";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FuerteCoinXtreme";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/FuerteCoinXtreme";
#else
    // Unix-like
    return pathRet / ".fuertecoinxtreme";
#endif
#endif
}
