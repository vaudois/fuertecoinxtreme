...
The FuerteCoinXtreme Core daemon can be configured to act as a trusted "border
router", implementing the fuertecoinxtreme wire protocol and relay, making
...
The ZeroMQ feature in FuerteCoinXtreme Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building fuertecoinxtremed:
...
For instance:
    ...
    $ fuertecoinxtremed -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/fuertecoinxtremed.tx.raw \
...
These options can also be provided in fuertecoinxtreme.conf.
...
From the perspective of fuertecoinxtremed, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into fuertecoinxtremed directly. Furthermore, no information is
...
using. FuerteCoinXtremed appends an up-counting sequence number to each
